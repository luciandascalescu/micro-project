...
void print_voltage(float measured_voltage){
	//float measured_voltage = ((analog_input * 3.3f) / 65535);
	//float measured_voltage = (((analog_input * 3.3f) / 65535)-3.2f)*100.0f;
...
void print_bin_reversed(uint16_t x){
	for(int i=0;i<16;i++){
			uint32_t one=1;
			UART0_Transmit( (((x)&(one<<i))>>i) + 0x30);
		}
		UART0_Transmit(0x0A);
		UART0_Transmit(0x0D);
}
